...
template <class TileShape_MN, class ClusterShape_MNK, class KernelSchedule,
          class EpilogueSchedule, int Stages>
...
// ----------------------------------------------------------------------------
using V16 = W4A8GroupedGemmKernel<Shape<_256, _16>, Shape<_1, _1, _1>, cutlass::gemm::KernelPtrArrayTmaWarpSpecializedPingpong, cutlass::epilogue::PtrArrayTmaWarpSpecializedPingpong, 3>;
void mm(torch::stable::Tensor& out,
  ...
  switch (variant) {
    case 16: V16::grouped_mm(out, a, b, token, channel, scale, group_size,
                  offsets, problems, astride, bstride, cstride, sstride, ptrs); break;
    default: STD_TORCH_CHECK(false, "Unknown stage variant");
  }
}

STABLE_TORCH_LIBRARY(h20_native_staged, m) {
...
STABLE_TORCH_LIBRARY_IMPL(h20_native_staged, CUDA, m) {
