...
namespace h20_native_pingpong_n8 {
...
using Coop = cutlass::gemm::KernelPtrArrayTmaWarpSpecializedPingpong;
using CoopEpi = cutlass::epilogue::PtrArrayTmaWarpSpecializedPingpong;
using K64 = W4A8GroupedGemmKernel<Shape<_64, _8>, Shape<_1, _1, _1>, Coop, CoopEpi>;
...
void mm(torch::stable::Tensor& out,
  ...
  if (tile == 64) {
    K64::grouped_mm(out, a, b, token, channel, scale, group_size, offsets,
                    problems, astride, bstride, cstride, sstride, ptrs);
  }
  else { STD_TORCH_CHECK(false, "Unsupported selected H20 tile"); }
}

STABLE_TORCH_LIBRARY(h20_native_pingpong_n8, m) {
...
STABLE_TORCH_LIBRARY_IMPL(h20_native_pingpong_n8, CUDA, m) {
