...
#include "resource_pingpong.hpp"

namespace h20_native_resources {
...
template <class TileShape_MN, class ClusterShape_MNK, class KernelSchedule,
          class EpilogueSchedule, int Stages, int LoadRegs, int MmaRegs, int MinBlocks, int GridFactor>
struct W4A8GroupedGemmKernel {
  ...
  using GemmKernelShuffled = cutlass::gemm::kernel::H20ResourceGemm<
      ProblemShape, CollectiveMainloopShuffled, CollectiveEpilogue,
      LoadRegs, MmaRegs, MinBlocks>;
  ...
  static std::string resource_info() {
    cudaFuncAttributes attr{};
    const auto error = cudaFuncGetAttributes(&attr, cutlass::device_kernel<GemmKernelShuffled>);
    STD_TORCH_CHECK(error == cudaSuccess, cudaGetErrorString(error));
    const int blocks = GemmShuffled::maximum_active_blocks();
    const int threads = GemmKernelShuffled::MaxThreadsPerBlock;
    const int requested = (LoadRegs + 2 * MmaRegs) * 128;
    // setmaxnreg.inc draws from a per-CTA pool. Refuse a pool that cannot
    // simultaneously satisfy all three warp groups before launching any work.
    STD_TORCH_CHECK(attr.numRegs * threads >= requested,
                    "Insufficient compiled CTA register pool for warp-group budgets");
    return std::string("{\"registers_per_thread\":") + std::to_string(attr.numRegs) +
        ",\"threads\":" + std::to_string(threads) +
        ",\"dynamic_smem_bytes\":" + std::to_string(GemmKernelShuffled::SharedStorageSize) +
        ",\"static_smem_bytes\":" + std::to_string(attr.sharedSizeBytes) +
        ",\"local_bytes_per_thread\":" + std::to_string(attr.localSizeBytes) +
        ",\"max_active_blocks_per_sm\":" + std::to_string(blocks) +
        ",\"requested_warp_group_registers\":" + std::to_string(requested) +
        ",\"grid_factor\":" + std::to_string(GridFactor) + "}";
  }
  ...
  static void grouped_mm(torch::stable::Tensor& out_tensors,
    ...
    STD_TORCH_CHECK(reinterpret_cast<uintptr_t>(prepared_ptrs.data_ptr()) % 16 == 0);
    ...
    // The grouped persistent scheduler uses sm_count as its logical CTA
    // budget, including tensor-map workspace sizing and work progression.
    // GridFactor scales this logical CTA budget while retaining the physical SM count.
    static const int physical_sms =
        cutlass::KernelHardwareInfo::query_device_multiprocessor_count(device_id);
    const cutlass::KernelHardwareInfo hw_info{device_id, physical_sms * GridFactor};
...
// ----------------------------------------------------------------------------
using V4 = W4A8GroupedGemmKernel<Shape<_128, _8>, Shape<_1, _1, _1>, cutlass::gemm::KernelPtrArrayTmaWarpSpecializedPingpong, cutlass::epilogue::PtrArrayTmaWarpSpecializedPingpong, 4, 40, 96, 2, 2>;
using V8 = W4A8GroupedGemmKernel<Shape<_256, _8>, Shape<_1, _1, _1>, cutlass::gemm::KernelPtrArrayTmaWarpSpecializedPingpong, cutlass::epilogue::PtrArrayTmaWarpSpecializedPingpong, 3, 32, 104, 2, 2>;
using V13 = W4A8GroupedGemmKernel<Shape<_256, _16>, Shape<_1, _1, _1>, cutlass::gemm::KernelPtrArrayTmaWarpSpecializedPingpong, cutlass::epilogue::PtrArrayTmaWarpSpecializedPingpong, 3, 32, 104, 2, 2>;
void mm(torch::stable::Tensor& out,
        ...
        const torch::stable::Tensor& ptrs, int64_t variant) {
  ...
  switch (variant) {
    case 4: V4::grouped_mm(out, a, b, token, channel, scale, group_size,
                  offsets, problems, astride, bstride, cstride, sstride, ptrs); break;
    case 8: V8::grouped_mm(out, a, b, token, channel, scale, group_size,
                  offsets, problems, astride, bstride, cstride, sstride, ptrs); break;
    case 13: V13::grouped_mm(out, a, b, token, channel, scale, group_size,
                  offsets, problems, astride, bstride, cstride, sstride, ptrs); break;
    default: STD_TORCH_CHECK(false, "Unknown resource variant");
  }
}


STABLE_TORCH_LIBRARY(h20_native_resources, m) {
  m.def("mm(Tensor! out, Tensor a, Tensor b, Tensor token, Tensor channel, "
        ...
        "Tensor ptrs, int variant) -> ()");
}
STABLE_TORCH_LIBRARY_IMPL(h20_native_resources, CUDA, m) {
