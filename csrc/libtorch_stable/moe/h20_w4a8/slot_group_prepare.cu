...
namespace h20_batch_fp8_groups {
...
__global__ void batch_fp8_prepare_kernel(
    ...
    int32_t* __restrict__ prob2, Addresses a, int tokens) {
  constexpr int Width = 2048, Hidden = 256, Experts = 256;
  constexpr int Pairs = Width / (Threads * 2);
  const int p = blockIdx.x, tid = threadIdx.x, rows = tokens * 8;
  const bool has_row = p < rows;
  const int expert = has_row ? static_cast<int>(ids[p]) : 0;
  __shared__ int destination;
  __shared__ float token_scale;
  // Retain the full stable expert prefix for callers. Only the first occurrence
  // of an expert submits its nonzero GEMM group; duplicate slots submit M=0.
  // This avoids a device-to-host active-group count and another compaction launch.
  if (tid < 32) {
    int prefix = 0, start = 0, count = 0, earlier = 0;
    for (int i = tid; i < rows; i += 32) {
      const int id = static_cast<int>(ids[i]);
      prefix += id < p;
      start += id < expert;
      count += id == expert;
      earlier += id == expert && i < p;
    }
    prefix = __reduce_add_sync(0xffffffffu, prefix);
    start = __reduce_add_sync(0xffffffffu, start);
    count = __reduce_add_sync(0xffffffffu, count);
    earlier = __reduce_add_sync(0xffffffffu, earlier);
    if (tid == 0) {
      offsets[p] = prefix;
      if (has_row) {
        destination = start + earlier;
        inv[p] = destination;
        perm[destination] = p;
        const bool valid = expert < Experts;
        const int e = valid ? expert : 0;
        const int64_t first_row = valid ? start : 0;
        const int active_count = valid && earlier == 0 ? count : 0;
        const int64_t first[6] = {
            a.y + first_row * Width, a.w1 + int64_t(e) * Width * Hidden,
            a.o1 + first_row * Hidden * 4, a.sy + first_row * 4,
            a.c1 + int64_t(e) * Hidden * 8,
            a.g1 + int64_t(e) * (Width / 128) * Hidden * 16};
        const int64_t second[6] = {
            a.q2 + first_row * Hidden, a.w2 + int64_t(e) * Hidden * Width / 2,
            a.o2 + first_row * Width * 2, a.s2 + first_row * 4,
            a.c2 + int64_t(e) * Width * 4,
            a.g2 + int64_t(e) * (Hidden / 128) * Width * 8};
#pragma unroll
        for (int j = 0; j < 6; ++j) {
          ptr1[j * rows + p] = first[j];
          ptr2[j * rows + p] = second[j];
        }
        prob1[3 * p] = Hidden * 2;
        prob1[3 * p + 1] = active_count;
        prob1[3 * p + 2] = Width;
        prob2[3 * p] = Width;
        prob2[3 * p + 1] = active_count;
        prob2[3 * p + 2] = Hidden;
      }
    }
  }
...
#pragma unroll
  ...
  const float aggregate = Reduce(storage).Reduce(maximum, Maximum{});
  if (tid == 0) {
    token_scale = fmaxf(aggregate / quant_type_max_v<Fp8>,
...
#pragma unroll
  for (int i = 0; i < Pairs; ++i) {
    const auto low = vllm::scaled_fp8_conversion<false, Fp8>(values[2 * i], token_scale);
    const auto high = vllm::scaled_fp8_conversion<false, Fp8>(values[2 * i + 1], token_scale);
    const uint16_t packed = uint16_t(*reinterpret_cast<const uint8_t*>(&low)) |
                            (uint16_t(*reinterpret_cast<const uint8_t*>(&high)) << 8);
    reinterpret_cast<uint16_t*>(y)[destination * (Width / 2) + tid + i * Threads] = packed;
  }
}
...
void run(const Tensor& input, const Tensor& ids, Tensor& y, Tensor& sy,
  ...
  STD_TORCH_CHECK(address(y) % 2 == 0, "FP8 output must support aligned two-byte stores");
  ...
  check(ptr1, input, Type::Long, 6 * rows);
  check(ptr2, input, Type::Long, 6 * rows);
  check(prob1, input, Type::Int, rows * 3);
  check(prob2, input, Type::Int, rows * 3);
...
#define LAUNCH(Index, Threads) \
  batch_fp8_prepare_kernel<Index, Threads><<<257, Threads, 0, stream>>>( \
      ...
      static_cast<int32_t*>(prob1.data_ptr()), static_cast<int32_t*>(prob2.data_ptr()), a, tokens)
...
STABLE_TORCH_LIBRARY(h20_batch_fp8_groups, m) {
...
STABLE_TORCH_LIBRARY_IMPL(h20_batch_fp8_groups, CUDA, m) {
