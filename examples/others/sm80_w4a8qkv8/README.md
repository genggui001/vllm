...
`marlin_fp8_qdq_fused` experts to vLLM 0.29.0. It retains the measured Prefill
...
- W4 compressed-tensors MoE weights and SILU/SwiGLU activation, with router
  weights applied after FC2. Clamped SwiGLU is rejected. Leave `VLLM_MARLIN_INPUT_DTYPE` unset.
...
shape. This padding policy applies to both V1 and V2 model runners.
The extra graph is opt-in; generic vLLM graph defaults are unchanged.
